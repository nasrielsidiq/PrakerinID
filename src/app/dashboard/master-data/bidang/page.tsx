...
const JurusanPage: React.FC = () => {
  ...
  const [fields, setFields] = useState<Province[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await API.patch(`${ENDPOINTS.FIELDS}/${editingId}`, formData, {
          headers: {
            Authorization: `Bearer ${Cookies.get("userToken")}`,
          },
        });
      ...
      if (editingId) {
        ...
        setEditingId(null);
        await alertSuccess("Bidang berhasil diubah!");
      } else {
        await alertSuccess("Bidang berhasil ditambahkan!", 1500);
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string, name: string) => {
    const confirm = await alertConfirm(
      `Apakah anda yakin ingin menghapus bidang "${name}"?`
    );
    if (!confirm) return;

    try {
      await API.delete(`${ENDPOINTS.FIELDS}/${id}`, {
        headers: {
          Authorization: `Bearer ${Cookies.get("userToken")}`,
        },
      });
      ...
      await fetchData();
      await alertSuccess(`Bidang ${name} berhasil dihapus!`);
  ...
  const handleAccept = async (id: string, name: string) => {
    const confirm = await alertConfirm(
      `Apakah anda yakin ingin menerima bidang "${name}"?`
    );
    if (!confirm) return;
    try {
      await API.patch(
      ...
      await fetchData();
      await alertSuccess(`Bidang ${name} berhasil diterima!`);
    } catch (error) {
      if (error instanceof AxiosError) {
        const responseError = error.response?.data.errors;
        await alertError(responseError);
  ...
  return (
    <main className="p-6 ">
      {/* Page Header */}
      <div className="mb-6">
        <h1 className="text-accent-dark text-sm mb-5">Bidang</h1>
        <div className="mb-8">
          <div className="flex items-center space-x-2 font-extrabold text-accent">
            <BriefcaseBusiness className="w-5 h-5" />
            <h2 className="text-2xl mt-2">Bidang</h2>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm">
        ...
        {fields.length === 0 && loading === false && (
          <div className="text-center py-12 col-span-2 ">
            <NotFoundComponent text="Tidak ada bidang yang ditemukan." />
          </div>
        )}
      </div>
          ...
          <div className="bg-white text-black p-6 rounded-lg flex flex-col gap-2 min-w-sm lg:min-w-xl">
            ...
            <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
              <div className="flex flex-col gap-2">
                <label htmlFor="province">Nama Bidang</label>
                <input
                  value={formData.name}
                  onChange={(e) =>
