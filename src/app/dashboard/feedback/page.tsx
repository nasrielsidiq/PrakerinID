'use client';
import { Building, Circle, CircleArrowRight, MapPin, MessageSquareText, X } from "lucide-react";
                ...
                <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                    <div className="p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-2xl font-bold text-teal-600">Beri Feedback</h2>
                            <button
        ...
        <main className="p-6 min-h-screen">
            ...
            <div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-black">
                    {
                        perusahaan && perusahaan.map((data, index) => (
                            <div className="bg-white flex flex-col md:flex-row space-x-5 p-5 px-10 md:px-5 rounded-2xl justify-between items-end md:items-center" key={index}>
                                <div className="flex w-full md:w-auto">
                                    <img src="/Makerindo_PS.png" alt="Icon" className="w-15 h-15" />
                                    <div className="ms-3 ">
                                        <h5 className="text-accent font-bold">{data.name}</h5>
                                        <span className="flex"><MapPin /> {data.kota}, {data.provinsi}</span>
                                    </div>
                                </div>
                                <button onClick={() => handleModal(index) } className="bg-vip/30 text-vip flex justify-between items-center p-1 px-2 space-x-2 rounded-full">
                                    <span>Feedback</span>
