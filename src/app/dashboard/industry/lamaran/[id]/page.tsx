...
const detailLamaran = ({ params }: { params: Promise<{ id: string }> }) => {
  const application: Application = {
    ...
    jobs: "Web Developer",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Inventore eius dignissimos officiis quo sequi, enim, et, reprehenderit porro aliquid aspernatur mollitia. Laboriosam repudiandae doloribus et.",
    cv: null,
  };
  const { id } = use(params);
    ...
    <main className="p-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center space-x-2 text-gray-600 mb-1">
                <span className="text-sm">
                  Kelas {application.kelas} | {application.jurusan}
                </span>
              </div>
        ...
        <div className="flex justify-end space-x-5">
          <button
            onClick={() => router.back()}
            className="p-3 px-5 bg-gray-300 text-gray-600 rounded-xl"
          >
            Kembali
          </button>
          <button
            onClick={() => router.push(`${id}/reject`)}
            className="p-3 px-5 bg-red-500 text-gray-100 rounded-xl"
          >
            Tolak Lamaran
          </button>
          <button
            onClick={() => router.push(`${id}/accept`)}
            className="p-3 px-5 bg-accent text-gray-100 rounded-xl"
          >
            Jadwalkan Test
          </button>
        </div>
