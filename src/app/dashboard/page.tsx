...
const Dashboard: React.FC = () => {
  ...
  const getStepColor = (stepIndex: number, currentStep: number, isCompleted: boolean) => {
  ...
  // const getConnectorColor = (stepIndex: number, currentStep: number) => {
  //   return stepIndex < currentStep ? 'bg-teal-500' : 'bg-gray-300';
  // };
    ...
    <main className="p-6">
      ...
      <div className=" p-6 mb-8">
        <div className="flex items-center space-x-2 font-bold text-accent">
          ...
          <h2 className="text-xl">Resume Lamaran Magang</h2>
        </div>
        <div className="flex items-center justify-between mt-4">
          <p className="text-gray-600 text-sm">Lihat! Kamu sudah apply 4 lowongan!</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {jobApplications.map((application) => (
          <div key={application.id} className="bg-white rounded-lg shadow-sm p-6">
            <div className="mb-4">
              <h3 className="font-semibold text-gray-800 mb-2">{application.title}</h3>
            ...
            <div className="relative">
              <div className="flex justify-between items-center">
                {application.steps.map((step, stepIndex) => (
                  <div key={stepIndex} className="flex flex-col items-center relative z-10">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-medium ${getStepColor(stepIndex, application.currentStep, step.completed)}`}>
                      {stepIndex + 1}
                    </div>
              ...
              <div className="absolute top-4 left-4 right-4 h-0.5 bg-gray-300 z-1">
                <div
                  ...
                  style={{ width: `${(application.currentStep / (application.steps.length - 1)) * 100}%` }}
                ></div>
              </div>
            </div>
          </div>
